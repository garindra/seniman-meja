...
export function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    switch (argument) {
      ...
      case "--allow-origin":
        {
          ...
          if (!hostname || hostname.includes("/") || hostname.includes(":") || hostname.includes("@") || hostname.includes("?") || hostname.includes("#") || !/^[a-z0-9.-]+$/.test(hostname)) {
            usage("--allow-origin requires a hostname without a scheme or port");
          }
      ...
      case "--listen":
        {
          const address = value();
          if (isIP(address) === 0 || address === "0.0.0.0" || address === "::") {
            usage("--listen requires a specific local IP address, not a wildcard");
          }
  ...
  if (!options.profile || !/^[A-Za-z0-9._-]+$/.test(options.profile) || options.profile === "." || options.profile === "..") {
