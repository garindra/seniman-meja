import { createHash, randomFillSync, timingSafeEqual, X509Certificate } from "node:crypto";
...
function encodeFrame(type, payload) {
  return Buffer.concat([encodeUvarint(type), encodeUvarint(payload.length), payload]);
}
...
function localSocketPath(options) {
  return options.socket || join(homedir(), `.meja/${options.profile}/meja.sock`);
}
...
function remoteForwardCommand(options) {
  const selector = options.socket ? ["-S", options.socket] : ["-L", options.profile];
  return [shellQuote(options.remotePath), ...selector.map(shellQuote), "__ssh-forward-v1"].join(" ");
}
async function resolveSSHConnectionHost(options) {
  const command = spawn("ssh", ["-G", ...sshOptions(options), options.sshHost], {
  ...
  await new Promise((resolve, reject) => {
    ...
    command.once("close", code => {
      if (code === 0) {
        ...
        reject(new Error(stderr.trim() || `ssh -G exited with status ${code ?? "unknown"}`));
      }
    });
  });
  for (const line of stdout.split("\n")) {
    const fields = line.trim().split(/\s+/);
    if (fields.length === 2 && fields[0].toLowerCase() === "hostname" && fields[1]) {
      return fields[1];
    }
  }
...
async function runMejaCommand(options, args) {
  ...
  return new Promise((resolve, reject) => {
      ...
      try {
        while (buffer.length >= 4) {
          ...
          if (frame.version !== COMMAND_PROTOCOL_VERSION) {
            throw new Error(`unsupported command protocol version ${frame.version}`);
          }
    ...
    if (options.sshHost) {
      const command = spawn("ssh", [...sshOptions(options), options.sshHost, remoteForwardCommand(options)], {
      ...
      command.once("close", code => {
        if (!settled) {
          ...
          finish(new Error(detail || (code === 0 ? "SSH command returned no exit frame" : `SSH command exited with status ${code ?? "unknown"}`)));
        }
      });
      ...
      socket.on("end", () => {
        if (!settled) {
          finish(new Error("Meja command socket closed without an exit frame"));
        }
      });
    }
  });
...
async function requestAttachmentGrant(options, args) {
  ...
  if (result.bootstrap.version !== COMMAND_BOOTSTRAP_VERSION) {
    throw new Error(`unsupported bootstrap version ${result.bootstrap.version}`);
  }
  return {
    bootstrap: result.bootstrap,
    connectionHost: options.sshHost ? await resolveSSHConnectionHost(options) : "127.0.0.1"
  };
}
...
class AsyncBytes {
  ...
  async exact(length) {
    ...
    while (written < length) {
      ...
      const buffer = this.buffers[0];
      const count = Math.min(length - written, buffer.length - this.offset);
...
async function readControlFrame(reader) {
  ...
  return {
    type,
    payload: await reader.exact(length)
  };
}
...
function decodeOSC52ClipboardWrite(payload) {
  ...
  if (payload.length < prefix.length + 3 || !payload.subarray(0, prefix.length).equals(prefix)) {
    return null;
  }
  let contentEnd;
  if (payload.length >= 2 && payload[payload.length - 2] === 0x1b && payload[payload.length - 1] === 0x5c) {
  ...
  const target = payload.subarray(prefix.length, separator).toString("ascii");
  if (!target || !/^[cps0-7]+$/.test(target)) {
    return null;
  }
  const encoded = payload.subarray(separator + 1, contentEnd).toString("ascii");
...
function decodeClientStatus(payload) {
  ...
  if (status.kind === STATUS_PROMPT && (status.prompt.promptId === 0 || status.prompt.mode < 1 || status.prompt.mode > 2)) {
...
async function readDisplayCommand(reader) {
  ...
  switch (opcode) {
    ...
    case DISPLAY_SCROLL_REGION:
      ...
      break;
    default:
      throw new Error(`unknown display opcode 0x${opcode.toString(16).padStart(2, "0")}`);
  }
  return command;
}
...
class PaneScreen {
  ...
  reset(cols, rows, revision) {
    ...
    this.dirtyRows = new Set(Array.from({
      length: rows
    }, (_, index) => index));
  ...
  clearOccupant(row, column) {
    ...
    if (previous.width === 2 && anchor + 1 < this.cols) {
      cells[anchor + 1] = blankCell(previous.styleId);
    }
  }
  writeCell(text, width, styleId) {
    if (width !== 1 && width !== 2 || this.row < 0 || this.row >= this.rows || this.column < 0 || this.column + width > this.cols) {
      throw new Error(`write at ${this.row},${this.column} width ${width} outside ` + `${this.cols}x${this.rows} grid`);
    }
  ...
  scrollRegion(top, bottom, delta) {
    ...
    if (!Number.isSafeInteger(top) || !Number.isSafeInteger(bottom) || !Number.isSafeInteger(delta) || top < 0 || top >= bottom || bottom > this.rows || delta === 0 || Math.abs(delta) > height) {
      throw new Error(`invalid scroll region [${top},${bottom}) delta ${delta} ` + `for ${this.cols}x${this.rows} grid`);
    }
    ...
    const replacement = delta < 0 ? region.slice(count).concat(blanks) : blanks.concat(region.slice(0, height - count));
    this.cells.splice(top, height, ...replacement);
    this.pendingScrollRegions.push({
      top,
      bottom,
      delta
    });
  ...
  apply(command) {
    switch (command.opcode) {
      ...
      case DISPLAY_WRITE_TEXT_UTF8_DEFAULT:
        {
          const styleId = command.opcode === DISPLAY_WRITE_TEXT_UTF8_DEFAULT ? 0 : this.styleId;
          for (const character of command.text) {
            this.writeCell(character === " " ? "" : character, command.width, styleId);
          }
          break;
        }
      case DISPLAY_WRITE_CLUSTER:
        ...
        break;
      case DISPLAY_FILL:
        for (let columns = 0; columns < command.columns; columns += command.width) {
          this.writeCell(command.rune === " " ? "" : command.rune, command.width, this.styleId);
        }
        break;
      case DISPLAY_CURSOR_UPDATE:
        {
...
function styleClass(slot, styleId, cursor) {
  return `mjp${slot}s${styleId}` + (cursor ? " __mjc" : "");
}
function rowSnapshot(screen, rowIndex) {
  ...
  for (let column = 0; column < screen.cols; column += 1) {
    ...
    const cursor = screen.cursor.visible && screen.cursor.y === rowIndex && screen.cursor.x === column;
...
function cellMatches(left, right) {
  ...
  return left.key === right.key && left.text === right.text && left.width === right.width;
}
...
function spanRuns(snapshot, start = 0, end = snapshot.cells.length) {
  ...
  for (let column = start; column < end; column += 1) {
    ...
    if (previous && previous.key === cell.key && previous.end === column && previous.spaceOnly === spaceOnly) {
      if (!spaceOnly) {
  ...
  feed(data, sourceIdle) {
    ...
    while (this.pending.length > 0) {
      if (this.pasting) {
        ...
        if (end < 0) {
          this.paste = combined;
          if (!this.pasteOverflow && Buffer.byteLength(this.paste, "utf8") > MAX_STRING_LENGTH + pasteEnd.length) {
        ...
        if (!this.pasteOverflow && Buffer.byteLength(pastedText, "utf8") <= MAX_STRING_LENGTH) {
          events.push({
            kind: "paste",
            text: pastedText
          });
...
class PromptDraft {
  ...
  snapshot(revision) {
    const caret = this.cursor < this.characters.length ? this.characters[this.cursor] : " ";
    return {
      revision,
      ...
      caret,
      after: this.cursor < this.characters.length ? this.characters.slice(this.cursor + 1).join("") : "",
  ...
  insertText(text, normalize) {
    ...
    for (let character of Array.from(text)) {
      if (normalize && (character === "\r" || character === "\n" || character === "\t")) {
        character = " ";
      }
      const code = character.codePointAt(0);
      if (code >= 0x20 && code !== 0x7f && this.insertCharacter(character)) {
        changed = true;
      }
    }
    return changed;
  ...
  consume(data, sourceIdle) {
    ...
    for (const event of this.decoder.feed(data, sourceIdle)) {
      if (this.descriptor.mode === 2) {
        if (event.kind === "rune" && (event.character === "y" || event.character === "Y")) {
          outcome.result = this.resolve(true, "y");
        } else if (event.kind === "rune" && (event.character === "n" || event.character === "N") || event.kind === "enter" || event.kind === "escape" || event.kind === "rune" && event.control && event.character === "c") {
      ...
      if (event.kind === "rune") {
        ...
        } else if (!event.control && this.insertText(event.character, false)) {
          outcome.changed = true;
        }
        ...
        outcome.changed = true;
      } else if (event.kind === "delete" && this.cursor < this.characters.length) {
        const [removed] = this.characters.splice(this.cursor, 1);
        this.textBytes -= Buffer.byteLength(removed, "utf8");
        outcome.changed = true;
        ...
        this.cursor -= 1;
        outcome.changed = true;
      } else if (event.kind === "right" && this.cursor < this.characters.length) {
        this.cursor += 1;
        outcome.changed = true;
      } else if (event.kind === "home" && this.cursor !== 0) {
        this.cursor = 0;
        outcome.changed = true;
      } else if (event.kind === "end" && this.cursor !== this.characters.length) {
        this.cursor = this.characters.length;
        outcome.changed = true;
...
function clientStatusMatches(left, right) {
  if (!left || !right || left.sessionId !== right.sessionId || left.sessionName !== right.sessionName || left.serverHostname !== right.serverHostname || left.serverHome !== right.serverHome || left.root !== right.root || left.kind !== right.kind || left.windows.length !== right.windows.length || left.prompt.promptId !== right.prompt.promptId || left.prompt.mode !== right.prompt.mode || left.prompt.label !== right.prompt.label || left.prompt.initial !== right.prompt.initial || left.message.id !== right.message.id || left.message.text !== right.message.text) {
    return false;
  }
  return left.windows.every((window, index) => {
    const candidate = right.windows[index];
    return window.windowId === candidate.windowId && window.index === candidate.index && window.title === candidate.title && window.active === candidate.active && window.zoomed === candidate.zoomed;
  });
}
class TerminalModel {
  ...
  setStatus(status) {
    ...
    if (status.sessionId > 0 && status.sessionId !== previousStatus.sessionId) {
      this.onSessionTarget?.(String(status.sessionId));
    }
    if (status.kind === STATUS_PROMPT) {
      if (!this.promptDraft || this.promptDraft.descriptor.promptId !== status.prompt.promptId) {
  ...
  present(slot, screen) {
    if (!this.renderAnnounced.has(slot) && screen.cols > 0 && screen.rows > 0) {
      this.renderAnnounced.add(slot);
      console.log(`seniman-meja: rendering pane slot ${slot} at ` + `${screen.cols}x${screen.rows}`);
    }
  ...
  metadata() {
    ...
    const focused = panes.find(({
      paneId
    }) => paneId === this.layout?.focusedPaneId) ?? panes[0];
    const cols = panes.reduce((maximum, pane) => Math.max(maximum, pane.rect.x + pane.rect.width), 0);
    const rows = panes.reduce((maximum, pane) => Math.max(maximum, pane.rect.y + pane.rect.height), 0);
    return {
      ...
      promptDraft: this.promptDraft?.snapshot(this.promptDraftRevision) ?? null
    };
  }
  snapshot() {
    return {
  ...
  paneSnapshot(slot) {
    ...
    return {
      styles: Array.from(screen.styles, ([id, style]) => styleRule(slot, id, style)),
      rows: Array.from({
        length: screen.rows
      }, (_, index) => rowSnapshot(screen, index))
    };
  }
  publish(reset, screen = null, includePendingScrollRegions = false, layoutChanged = false) {
    const event = {
      slot: screen?.slot ?? null,
      layoutChanged,
      reset,
      scrollRegions: screen && includePendingScrollRegions && !reset ? screen.pendingScrollRegions.slice() : [],
      styleReset: screen?.styleResetPending ?? false,
      styles: screen ? screen.pendingStyleInstalls.map(({
        id,
        style
  ...
  activeWindow() {
    return this.status.windows.find(({
      active
    }) => active) ?? null;
  }
  ...
  scheduleMobileZoomReconcile() {
    ...
    queueMicrotask(() => {
      this.zoomReconcileScheduled = false;
      this.reconcileMobileZoom().catch(error => this.setError(error));
    });
  }
  async reconcileMobileZoom() {
    if (this.zoomCommandInFlight || !this.inputSender || !this.layout) {
      return;
    }
    ...
    const compact = this.requestedCols < COMPACT_MULTI_PANE_COLUMNS;
    ...
    } else if (state?.phase === "unzoom-requested" && !window.zoomed) {
    ...
    if (compact) {
      ...
      await this.sendMobileZoomToggle(key, state, `zoom pane ${state.paneId}`);
      return;
    }
    if (!state?.owned) {
      return;
  ...
  async sendResize(cols = this.requestedCols, rows = this.requestedRows) {
    if (!this.resizeSender || !Number.isSafeInteger(cols) || !Number.isSafeInteger(rows) || cols <= 0 || rows <= 0) {
      return;
    }
  ...
  async selectWindow(windowId) {
    const window = this.status.windows.find(candidate => candidate.windowId === windowId);
    if (!window || window.active) {
      return;
    }
    const target = String(window.index);
    const input = window.index >= 0 && window.index <= 9 ? `\x02${target}` : `\x02:select-window -t :${target}\r`;
    await this.sendInput(input, false);
  }
  async createWindow() {
    await this.sendInput("\x02c", false);
  }
  async sendWheel(buttonCode, column, row, modifiers, reportCount) {
    const pane = paneAt(this.layout?.panes ?? [], column, row);
    if (!pane || ![64, 65, 66, 67].includes(buttonCode) || !Number.isSafeInteger(column) || !Number.isSafeInteger(row) || !Number.isSafeInteger(modifiers) || !Number.isSafeInteger(reportCount)) {
      return;
    }
    const x = column + 1;
    const y = row + 1;
    const sequence = `\x1b[<${buttonCode + modifiers};${x};${y}M`;
    await this.sendInput(sequence.repeat(Math.min(Math.max(reportCount, 1), 24)));
  }
  async sendPointer(action, button, column, row, modifiers) {
    const pane = paneAt(this.layout?.panes ?? [], column, row);
    if (!pane || !["press", "move", "release"].includes(action) || !Number.isSafeInteger(button) || button < 0 || button > 2 || !Number.isSafeInteger(column) || !Number.isSafeInteger(row) || !Number.isSafeInteger(modifiers)) {
      return;
    }
    const final = action === "release" ? "m" : "M";
    const buttonCode = action === "move" ? button + 32 : button;
    const sequence = `\x1b[<${buttonCode + modifiers};${column + 1};${row + 1}${final}`;
...
async function consumeControlStream(reader, model, executeTerminalExit) {
  while (true) {
    ...
    } else if (frame.type === MSG_FRONTEND_REGISTER_TERMINAL_EXIT_COMMAND) {
      // Browser rendering has no physical terminal state to restore.
    } else if (frame.type === MSG_FRONTEND_EXECUTE_TERMINAL_EXIT_COMMAND) {
      await executeTerminalExit(decodePayloadString(frame.payload));
      return;
    }
  }
}
async function attachMeja(bootstrap, connectionHost, options, model, onClose, onTerminalExit) {
  ...
  const client = await QUICClient.createQUICClient({
    ...
    config: {
      ...
      verifyCallback: async certs => {
        pinVerified = certificateMatchesPin(certs, bootstrap.certSpkiSha256);
        return pinVerified ? undefined : native.CryptoError.BadCertificate;
      }
    }
  }, {
    timer: 10_000
  ...
  client.connection.addEventListener(events.EventQUICConnectionError.name, handleConnectionError, {
    once: true
  });
  const streamTasks = new Set();
  const track = promise => {
    streamTasks.add(promise);
    promise.catch(error => model.setError(error)).finally(() => streamTasks.delete(promise));
  };
  client.connection.addEventListener(events.EventQUICConnectionStream.name, event => {
  ...
  const attachPayload = Buffer.concat([encodeString(bootstrap.attachToken), encodeUvarint(options.cols), encodeUvarint(options.rows)]);
  ...
  if (attachResult.type === MSG_SESSION_ATTACH_FAILED) {
    ...
    throw new Error(`SESSION_ATTACH_FAILED: ${decodePayloadString(attachResult.payload)}`);
  }
  if (attachResult.type !== MSG_SESSION_ATTACH_OK) {
    ...
    throw new Error(`unexpected session attachment result ${attachResult.type}`);
  }
  ...
  model.setInputSender(async (data, sourceIdle) => {
    ...
    const payload = Buffer.concat([encodeUvarint(layoutRevision), Buffer.from([sourceIdle ? 1 : 0]), Buffer.from(data, "utf8")]);
    await writeControlFrame(MSG_FRONTEND_INPUT_BYTES, payload);
  });
  model.setPromptResultSender(async result => {
    if (!Number.isSafeInteger(result.promptId) || result.promptId <= 0 || Buffer.byteLength(result.text, "utf8") > MAX_STRING_LENGTH) {
      throw new Error("invalid FRONTEND_PROMPT_RESULT");
    }
    const payload = Buffer.concat([encodeUvarint(result.promptId), Buffer.from([result.submitted ? 1 : 0]), encodeString(result.text)]);
    await writeControlFrame(MSG_FRONTEND_PROMPT_RESULT, payload);
  });
  model.setResizeSender(async (cols, rows) => {
    const payload = Buffer.concat([encodeUvarint(cols), encodeUvarint(rows)]);
    await writeControlFrame(MSG_FRONTEND_RESIZE, payload);
  });
  track(consumeControlStream(controlReader, model, async message => {
    await writeControlFrame(MSG_FRONTEND_TERMINAL_EXIT_COMPLETE, Buffer.alloc(0));
  ...
  return {
    ...
    async destroy() {
      ...
      locallyClosing = true;
      model.disconnect();
      client.connection.removeEventListener(events.EventQUICConnectionError.name, handleConnectionError);
      await client.destroy({
        force: true
      }).catch(() => {});
    }
...
export { STATUS_MESSAGE, STATUS_PROMPT, SESSION_REPLACED_ERROR_CODE, TerminalModel, attachMeja, cellMatches, paneAt, requestAttachmentGrant, runMejaCommand, spanRuns };